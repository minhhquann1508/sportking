...
class Comment extends Database
{
    ...
    public function get_comment_by_product_id($product_id)
    {
        $query = "  SELECT c.*,u.email,u.fullname,u.phone,u.role
                    FROM comments c
                    INNER JOIN users u ON c.user_id = u.user_id
                    WHERE product_id = ?";
        $result = $this->select($query, [$product_id]);
        if ($result) {
            return ['success' => true, 'message' => 'Get thành công', 'data' => $result];
        } else {
            return ['success' => false, 'message' => 'Get thất baị', 'data' => null];
        }
    }
